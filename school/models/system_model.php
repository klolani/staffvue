...
class System_model extends CI_Model
{
    ...
    public function set_system_info_to_session()
    {
        ...
        if ($result) {
            ...
            $this->session->set_userdata('brand_sologan', $result->brand_slogan);
    ...
    public function update_system_info()
    {
        ...
        switch ($name) {
            case 'brand_name':
                ...
                $data['brand_name'] = $this->input->post('value', TRUE);
                break;
            case 'slogan':
                if ($this->session->userdata['user_role_id'] != 1) {
                    return FALSE;
                }
                $data['brand_slogan'] = $this->input->post('value', TRUE);
                break;
            case 'bussiness_type':
                $data['commercial'] = $this->input->post('value', TRUE);
                break;
            case 'email':
                $data['support_email'] = $this->input->post('value', TRUE);
                break;
            case 'phone':
                $data['support_phone'] = $this->input->post('value', TRUE);
                break;
            case 'youtube':
            ...
            case 'flickr':
                ...
                break;
            case 'show_video':
                $data['show_video_on_home'] = $this->input->post('value', TRUE);
                break;
            case 'can_register':
                $data['student_can_register'] = $this->input->post('value', TRUE);
                break;
            default:
                return FALSE;
                break;
    ...
    public function update_faq()
    {
        ...
        $data['faq_last_update'] = date('Y-m-d');
        $this->db->where('faq_id', (int) $this->input->post('pk'));
        if ($name == 'faq_title') {
            $data['faq_ques'] = $this->input->post('value', TRUE);
        } elseif ($name == 'faq_ans') {
            $data['faq_ans'] = $this->input->post('value', TRUE);
        } else {
            return FALSE;
        }
        $data['faq_last_update'] = date('Y-m-d');
