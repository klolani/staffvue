...
class Login_control extends CI_Controller
{
    ...
    public function register($message = '')
    {
        if ($_POST) {
            ...
            $this->form_validation->set_rules('user_email', 'Email Address', 'required|valid_email');
            $this->form_validation->set_rules('user_pass', 'Password', 'required|min_length[8]|matches[user_passcf]');
            $this->form_validation->set_rules('user_passcf', 'Confirm Password', 'required|min_length[8]');
            if ($this->form_validation->run() != FALSE) {
                ...
                if ($this->login_model->register($info)) {
                    ...
                    $to = $info['user_email'];
                    $suject = 'Account activation with ' . $this->session->userdata['brand_name'];
