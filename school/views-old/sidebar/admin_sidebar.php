...
<ul class="nav sidebar">
    ...
    <li><a href="#" class="sub <?=($active==2)?"active":'';?>"><i class="fa fa-bullseye"></i> Exam Control</a>
        <ul>
            ...
            <?php if ($this->session->userdata['user_role_id'] <= 3) { ?>
            <li><a href="<?=base_url('admin_control/view_categories');?>" class="<?=($class==23)?"current":'';?>">View Categories</a></li>
            <?php } ?>
            <li><a href="<?=base_url('create_category');?>" class="<?=($class==24)?"current":'';?>">Create New Category</a></li>
    ...
    <?php } ?>
    <?php if ($this->session->userdata['user_role_id'] <= 2) { ?>
        <li><a class="<?=($active==6)?"active":'';?>" href="<?=base_url('noticeboard'); ?>"><i class="fa fa-bookmark"></i> Noticeboard</a></li>
    <?php } ?>    
    <?php if ($this->session->userdata['user_role_id'] <= 3) { ?>
    ...
    <?php } else {?>
        <li><a class="<?=($active==4)?"active":'';?>" href="<?=base_url('message_control'); ?>"><i class="fa fa-envelope-o"></i> Inbox</a></li>
    <?php }?>
    <?php if ($this->session->userdata['user_role_id'] <= 3) { ?>
        <li><a class="<?=($active==5)?"active":'';?>" href="<?=base_url('admin_control/view_payment_history');?>"><i class="fa fa-money"></i> Payment History</a></li>
    <?php }?>
